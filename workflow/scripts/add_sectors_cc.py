...
if __name__ == "__main__":
    ...
    # CC: If need to insert existing heating capacity...
    ind_existing_path = snakemake.input.ind_existing_cap
    bus_gis_path = snakemake.input.bus_gis
    ...
    build_heat(
        ...
        ind_existing_path = ind_existing_path, 
        bus_gis_path = bus_gis_path, 
        busmap_path = busmap_path, 
        dynamic_pricing=True,
        sectors_include = ['ind', 'ind-naics'],
        eia=eia_api,
        year=dynamic_cost_year,
    )

    if snakemake.params.geothermal_heating: 
        from build_geothermal_heat import build_geothermal_heat
        cost_direct_use_path = snakemake.input.direct_use
        regions_onshore_path = snakemake.input.regions_onshore
        geo_egs_sc_path = snakemake.input.geo_egs_sc

        build_geothermal_heat(
            n=n,
            cost_direct_use_path = cost_direct_use_path, 
            region_cluster_path = regions_onshore_path, 
            geo_egs_sc_path = geo_egs_sc_path,
        )
